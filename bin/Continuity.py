...
DSUB_pins = {
    "Group  1": [
        [1, 12, 0],
        [2.4e3, 2.4e3, 2.4e3, 2.4e3, 2.4e3, 2.4e3],
    ],  # start from 1 because zero is special
    ...
    while press != ord("j"):
        ...
        for key in DSUB_pins:
            ...
            bin1 = [
                int(x)
                for x in list(
                    "{0:04}".format(int("{0:0b}".format(DSUB_pins[key][0][0])))
                )
            ]
            bin2 = [
                int(x)
                for x in list(
                    "{0:04}".format(int("{0:0b}".format(DSUB_pins[key][0][1])))
                )
            ]
            bin1.reverse()
            bin2.reverse()
            ...
            stdscr.addstr(
                line,
                offset,
                key + "\t" + f"{R:9.1f}" + " Ohm\t" + f"{target:6.1f}" + " Ohm",
